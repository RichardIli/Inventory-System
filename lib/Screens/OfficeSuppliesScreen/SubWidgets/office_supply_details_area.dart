...
import 'package:inventory_system/Screens/OfficeSuppliesScreen/SubWidgets/restock_window.dart';
import 'package:inventory_system/bloc/OfficeSuppliesScreenBlocs/OfficeSuppliesBloc/office_supplies_bloc.dart';
import 'package:inventory_system/bloc/OfficeSuppliesScreenBlocs/OfficeSupplyHistoryBloc/office_supply_history_bloc.dart';
...
class OfficeSupplyDetailsArea extends StatelessWidget {
  const OfficeSupplyDetailsArea({
    super.key,
  ...
  Widget build(BuildContext context) {
    final MyFirebaseAuth authDbRepo =
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (context) => RestockOfficeSupplyButtonBloc(
              auth: authDbRepo,
              firestoreOfficeSupplies:
                  RepositoryProvider.of<FirestoreOfficeSupplies>(context)),
        ),
        BlocProvider(
            create: (context) => OfficeSupplyHistoryBloc(
                firestoreOfficeSupplies:
                    RepositoryProvider.of<FirestoreOfficeSupplies>(context))),
      ],
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        ...
                        DetailsField(
                            fieldLabel: "NAME", fieldValue: supplyName),
                        BlocListener<RestockOfficeSupplyButtonBloc,
                            RestockOfficeSupplyButtonState>(
                          listener: (context, state) {
                            if (state is RestockOfficeSupplyButtonLoaded) {
                              if (state.success) {
                                context
                          ...
                          child: BlocBuilder<OfficeSuppliesBloc,
                              OfficeSuppliesState>(
                            builder: (context, state) {
                              if (state is OfficeSuppliesInitial) {
                                ...
                                final rawSupplyData = state.data
                                    .where((item) => item["id"] == supplyId);
                                final supplyAmount =
                                    rawSupplyData.first["amount"];
                                final supplyUnit = rawSupplyData.first["unit"];
                          ...
                          child: TextButton.icon(
                            onPressed: () {
                              ...
                              showDialog(
                                  ...
                                  builder: (context) {
                                    return OfficeSupplyRestockWindow(
                    ...
                    Column(
                      children: [
                        ...
                        BlocBuilder<SaveQrCodeButtonBloc,
                ...
                Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: BlocListener<RestockOfficeSupplyButtonBloc,
                          RestockOfficeSupplyButtonState>(
                        listener: (context, state) {
                          if (state is RestockOfficeSupplyButtonLoaded) {
                            if (state.success) {
                              context.read<OfficeSupplyHistoryBloc>().add(
                                  FetchOfficeSupplyHistoryEvent(
                                      supplyID: supplyId));
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text('Insertion failed')),
                        ...
                        child: Column(
                          children: [
                            ...
                            BlocBuilder<OfficeSupplyHistoryBloc,
                                OfficeSupplyHistoryState>(
                              builder: (context, state) {
                                if (state is OfficeSupplyHistoryInitial) {
                                  context.read<OfficeSupplyHistoryBloc>().add(
                                      FetchOfficeSupplyHistoryEvent(
                                          supplyID: supplyId));
                                  return Center(
                                    ...
                                    is OfficeSupplyHistoryLoading) {
                                  return Center(
                                      child: CircularProgressIndicator());
                                } else if (state is OfficeSupplyHistoryLoaded) {
                                  return Expanded(
                                      ...
                                      itemBuilder: (context, index) {
                                        ...
                                        final String outBy =
                                            item["outBy"] != null
                                                ? item["outBy"].toString()
                                                : "";
                                        final String outDate = item[
                                                    "releaseDate"] !=
                                                null
                                            ? "${((item["releaseDate"].toDate()).year).toString()}-${((item["releaseDate"].toDate()).month).toString()}-${((item["releaseDate"].toDate()).day).toString()}"
                                            : "";
                                        final String inDate = item["inDate"] !=
                                                null
                                            ? "${((item["inDate"].toDate()).year).toString()}-${((item["inDate"].toDate()).month).toString()}-${((item["inDate"].toDate()).day).toString()}"
                                            : "";
                                        ...
                                        return Row(
                                          children: [
                                            ...
                                            ListContent(displayText: inDate),
                                            ListContent(displayText: inAmount),
                                            ListContent(displayText: outBy),
                                            ListContent(displayText: outDate),
                                            ListContent(displayText: outAmount),
                                    ...
                                    is OfficeSupplyHistoryStateError) {
